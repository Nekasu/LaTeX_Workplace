...
\begin{tikzpicture}
    ...
    \node at ($(Rceo)+(0,-\widthRceo/2-0.3)$) (Rceo_text) {$R_{ce\_o}$};
    ...
    \node at ($(Sb)+(0,-\widthSb/2-0.3)$) (Sb_text) {$S_b$};
    ...
    \node at ($(Rb)+(0,-\widthRb/2-0.3)$) (Rb_text) {$R_{b}$};
    ...
    \node at ($(Sbg)+(0,-\widthSbg/2-0.3)$) (Sbg_text) {$S_{bg}$};
    ...
    \node at ($(Oo)+(0,-\widthOo/2-0.3)$) (Oo_text) {$O_{o}$};

    
    % 绘制箭头
    % Sceo上的出度箭头1：Sceo->Rb
    \draw[->, thick] (Rceo_right) -- (Rb_left) node[midway, above] {Gaussian} node[midway, below] {Blur};

    % Sm上的出度箭头1：Sm->Sbg
    \draw[->, thick] (Sb_right) -- (Sbg_left) node[midway, above] {VGG19};

    % Rb 上的出度箭头1：Rb_AdaIN
    \draw[->, thick] (Rb_right) -- ($(Rb_right)+(0.4,0)$) |- (AdaIN_left);

    % Sbg 上的出度箭头1：Sbg_AdaIN
    \draw[->, thick] (Sbg_right) -- ($(Sbg_right)+(0.4,0)$) |- (AdaIN_left);
