...
\begin{tikzpicture}
    ...
    % 一个将频域处理的四张图像包裹起来的矩形
    \draw[fill=6ecd32!10, rounded corners=5pt] (-2,-3) rectangle (6,2.5);
    % % 一个用于表示原始风格图像的node
    \coordinate (C) at (0,0);
    ...
    \node at ($(Hdenoised)+(0,-\widthHdenoised/2-0.5)$) (Hdenoised_text) {$H_{\rm{denoised}}$};
    ...
    \node at ($(Ccanonical)+(0,-\widthCcanonical/2-0.5)$) (Ccanonical_text) {$C_{\rm{canonical}}$};
    ...
    \node at ($(mid_right_bottom)+(-0.08,-1)$) (DE_bottom) {$Depth\\Estimation$};

    %创建一个组圆角矩形, 表示深度估计的结果depth
    \coordinate (depth) at ($(last_front)+(\offX,0)$);
    \def\widthdepth{1.2};
    \rSquareSet{1}{GanYu_ricewhite}{50}{2pt}{13}{-0.5}{0.2}{DN}{0.1};
    \node at ($(DN_bottom)-(0,0.5)$) (DN_text) {$D_N$};

    %一个将右侧四个矩阵包裹起来的矩形
    \draw[fill=NekasuBlue!20, rounded corners=5pt] (15.2,-5) rectangle (17.8,5);
    \node at (16.5,-5) (verticleRec_bottom) {};
    %一个将右侧四个矩阵中, 上面两个包裹起来的矩形
    \draw[fill=GanYu_lightgray!20, rounded corners=5pt] (15.5,0.2) rectangle (17.5,4.5);
    %一个将右侧四个矩阵中, 下面两个包裹起来的矩形
    \draw[fill=GanYu_midblue!20, rounded corners=5pt] (15.5,-0.3) rectangle (17.5,-4.7);
    \node at (16.5,-5) (verticleRec_bottom) {};
    ...
    \node at ($(Mi)+(0,-\widthMi/2-0.5)$) (Mi_text) {$M_{i}$};
    %为Mi的方格涂上颜色
        %(0,0)
    ...
    \coordinate (Mia) at ($(Mi)+(0,-2*\offY/3)$);
    \def\widthMia{1.2};
    ...
    \node at ($(Mia)+(0,-\widthMia/2-0.5)$) (Mia_text) {$M_{i}^{\alpha}$};
    %为Mia的方格涂上颜色
        %(0,0)
    ...
    \node at ($(Mo)+(0,-\widthMo/2-0.5)$) (Mo_text) {$M_{o}$};
    %为Mo的方格涂上颜色
        %(0,0)
    ...
    \coordinate (Moa) at ($(Mo)+(0,-2*\offY/3)$);
    \def\widthMoa{1.2};
    ...
    \node at ($(Moa)+(0,-\widthMoa/2-0.5)$) (Moa_text) {$M_{o}^{\alpha}$};
    %为Moa的方格涂上颜色
        %(0,0)
    ...
    \node at ($(Rcei)+(0,-\widthRcei/2-0.5)$) (Rcei_text) {$R_{ce\_i}$};

    %创建一个方形, 本阶段结果2：Rceo
    \coordinate (Rceo) at ($(Rcei)+(0,-2)$);
    \def\widthRceo{1.2};
    ...
    \node at ($(Rceo)+(0,-\widthRceo/2-0.5)$) (Rceo_text) {$R_{ce\_o}$};
    ...
    \draw[->, thick] (oplus.east) -- (Ccanonical_left) node[midway, above] {$D^{-1}$} node[midway, below] {$\tau_c$};
